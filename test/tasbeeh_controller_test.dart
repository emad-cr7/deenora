...
void main() {
  group('TasbeehController Tests', () {
    ...
    test(
      'Preserves nullable narratedCount without fabricating numbers',
      () async {
        final controller = TasbeehController();
        await controller.loadDhikr();

        // First dhikr: SubhanAllah (narratedCount: 33)
        final firstDhikr = controller.dhikrList.firstWhere(
          (d) => d.id == 'subhanallah',
        );
        ...
        final nullCountDhikr = controller.dhikrList.firstWhere(
    ...
    test('Counter stops at narrated target (33 cannot become 34)', () async {
      ...
      final subhanAllah = controller.dhikrList.firstWhere(
        (d) => d.id == 'subhanallah',
      );
    ...
    test(
      'Custom target stops at target, open-ended continues without limit',
      () async {
        final controller = TasbeehController();
        await controller.loadDhikr();

        // Open-ended Dhikr (subhanallahil-azim has narratedCount == null)
        final openDhikr = controller.dhikrList.firstWhere(
          (d) => d.id == 'subhanallahil-azim',
        );
    ...
    test(
      'Custom Dhikr: validation, creation, selection, and counter stop',
      () async {
        final controller = TasbeehController();
        await controller.loadDhikr();
        ...
        expect(controller.addCustomDhikr(text: '   ', count: 33), isFalse);
        expect(
          controller.addCustomDhikr(text: 'Alhamdulillah', count: 0),
          isFalse,
        );
        expect(
          controller.addCustomDhikr(text: 'Alhamdulillah', count: -10),
          isFalse,
        );
